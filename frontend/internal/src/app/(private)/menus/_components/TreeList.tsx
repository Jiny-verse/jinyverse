...
function TreeLevel({
  ...
  return (
    <ul className={level > 0 ? 'ml-4 border-l border-border pl-1' : ''}>
      {nodes.map((node) => {
          ...
          <li key={node.id} className="py-0.5">
            <button
              ...
              className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition-colors ${
                isSelected ? 'bg-card text-foreground' : 'text-gray-700 hover:bg-gray-100'
              }`}
            ...
            >
              ...
              <span className={`shrink-0 text-xs ${isSelected ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                {node.code}
              </span>
            </button>
  ...
  if (tree.length === 0) {
    return (
      <div className="flex min-h-[280px] flex-col items-center justify-center py-14 text-center">
        ...
        <p className="text-sm font-medium text-muted-foreground">메뉴가 없습니다</p>
        <p className="mt-1 text-xs text-muted-foreground">메뉴 추가로 등록하세요.</p>
      </div>
    );
  }
  return (
