...
export function LandingCanvas({ className }: LandingCanvasProps) {
  const { t } = useLanguage();
  const { sections, reorderSection, openAddSectionModal, viewportMode } = useLandingContext();
  ...
  return (
    <div className={`relative ${className ?? ''}`} onDragEnd={handleDragEnd}>
      <div
        className={`transition-all duration-300 ${
          viewportMode === 'mobile' ? 'max-w-[430px] mx-auto border-x border-border shadow-lg' : ''
        }`}
      >
        {sections.length === 0 ? (
